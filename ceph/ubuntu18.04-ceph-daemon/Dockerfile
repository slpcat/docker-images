...
RUN \
    ...
    && apt-get update -q \
    && DEBIAN_FRONTEND=noninteractive apt-get install -yq apt-utils dialog libjemalloc1 iproute2 tzdata \ 
...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get install -y wget unzip uuid-runtime python-setuptools udev dmsetup && \
  ...
  echo "deb http://download.ceph.com/nfs-ganesha/deb-V2.6-stable/mimic/ bionic main" | tee /etc/apt/sources.list.d/nfs-ganesha.list && \
